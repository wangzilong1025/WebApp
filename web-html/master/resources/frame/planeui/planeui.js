...
(function ($) {
    ...
    dialog.creater.fn = dialog.creater.prototype = { 
        ...
        content : function(options) {
            ...
            else
            {
                if (type === "iframe")
                {
                    ...
                    //     if(height.indexOf("%") > 0){
                    //         height = $(window).height() * ((parseFloat(height)-0.03) / 100);
                    //     }else{
                    ...
                    // }
                    padding = 0;
                    content = '<iframe src="'+(options.url || "")+(options.url.indexOf('?') > 0 ? "&" : "?")+'_dialog='+this.id+'" id="iframe-'+this.id+'" dialogId="'+this.id+'" width="100%" height="100%" frameborder="0" scrolling="auto"></iframe>';
                }
                ...
                if (type === "confirm-app") 
                {
                    html += '<div class="pui-btn-group pui-btn-group-justify"><a href="javascript:;" button="yes" class="pui-btn pui-btn-default '+(buttonsClass.yes || '')+'">'+buttons.values.yes+'</a><a href="javascript:;" button="cancel" class="pui-btn pui-btn-default '+(buttonsClass.cancel || '')+'">'+buttons.values.cancel+'</a></div>';
                }
                ...
                if(type === "image"){
                    var imgWidth = settings.width;
                    var imgHeight = settings.height;
                    var tempHtml = "";

                    if(imgWidth!=null && imgWidth!=""){
                        if(imgWidth.indexOf("%") > 0){
                            imgWidth = ($(window).width() * (parseFloat(imgWidth) / 100)-90) + "px";
                        }else{
                            imgWidth = parseInt(imgWidth)-90 + "px";
                        }

                        tempHtml = "width='"+imgWidth+"' ";
                    }

                    if(imgHeight!=null && imgHeight!=""){
                        if(imgHeight.indexOf("%") > 0){
                            imgHeight = ($(window).height() * (parseFloat(imgHeight) / 100)-40)+"px";
                        }else{
                            imgHeight = parseInt(imgHeight)-40 + "px";
                        }
                        tempHtml += " height='"+imgHeight+"'";
                    }

                    if(typeof(imageURL) === 'string'){
                        var html = "<img src='"+imageURL+"' class='pui-size-auto' "+tempHtml+"/>";
                        target.children('section').html(html);
                        target.trigger('dialog.reset');
                    }else{
                        var html = "<img src='"+imageURL+"' class='pui-size-auto' imageurl='"+imageURL.join(",")+"' "+tempHtml+" currIdx=0 pui-image-src/>";
                        if(imageURL.length>1){
                            html = "<a href='javascript:;' pui-image-left class='pui-disabled' style='margin-right:5px;'><i class='fa fa-chevron-left fa-2x'></i></a>"
                            + html
                            +"<a href='javascript:;' pui-image-right style='margin-left:5px;'><i class='fa fa-chevron-right fa-2x'></i></a>";
                        }
                        target.children('section').html(html);
                        target.trigger('dialog.reset');
                    }
            ...
            if(type==="image"){
                target.find("a[pui-image-left]").bind($.clickOrTouch(),function(){
                    var img = target.find("[pui-image-src]")
                    var imageurl = img.attr("imageurl");
                    if(imageurl){
                        var currIdx = parseInt(img.attr("currIdx"))-1;
                        var imageurls = imageurl.split(",");
                        img.attr("src",imageurls[currIdx]);
                        img.attr("currIdx",currIdx);
                        if(currIdx==0){
                            target.find("a[pui-image-left]").addClass("pui-disabled");
                        }
                        target.find("a[pui-image-right]").removeClass("pui-disabled");
                        //target.trigger('dialog.reset');
                    }
                });
                target.find("a[pui-image-right]").bind($.clickOrTouch(),function(){
                    var img = target.find("[pui-image-src]")
                    var imageurl = img.attr("imageurl");
                    if(imageurl) {
                        var currIdx = parseInt(img.attr("currIdx")) + 1;
                        ...
                        img.attr("src", imageurls[currIdx]);
                        img.attr("currIdx",currIdx);
                        if (currIdx == imageurls.length-1) {
                            target.find("a[pui-image-right]").addClass("pui-disabled");
                        }
                        target.find("a[pui-image-left]").removeClass("pui-disabled");
                        //target.trigger('dialog.reset');
                    }
                });
            }
        ...
        close : function(callback) {
            ...
            var retVal = "";
            try {
                if(this.settings.dataReturn && this.settings.dataReturn != ''){
                    retVal = $("#" + this.id).find("iframe").contents().find("#" + this.settings.dataReturn).val();
                }
            } catch (e) {
...
(function($){
    ...
    $.fn.tooltip = function(options) {
        ...
        var defaults = {
            ...
            addClass : "pui-tooltip-bordered pui-tooltip-secondary-light",
            cached : false,
            ajax : new Function,
            showMode : "show",
            touch : true,
